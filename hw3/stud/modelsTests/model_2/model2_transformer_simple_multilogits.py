...
class Model2_net(nn.Module):
    ...
    def forward(
        ...
        transformer_out = torch.stack(
            transformer_outs.hidden_states[-4:],
            dim=0).sum(dim=0)
...
try:
    from .modelsTests.dataset.NERDataset_transformer_simple import NERDataset_transformer_simple
    from .modelsTests.utils.Trainer_model2_transformer_simple_multilogits import Trainer_model2_transformer_simple_multilogits
except: # notebooks
...
class Model2():
    def __init__(
        ...
        self.trainer = Trainer_model2_transformer_simple_multilogits()
        ...
        model_save_file_path = os.path.join(saves_path,'model2_weights_transformer_simple_multilogits.pth') if model_save_file_path is None else model_save_file_path

        self.model = Model2_net( 
        ...
        self.dataset = NERDataset_transformer_simple(
            data_path = None,
    ...
    def predict(self, sentence):
        ...
        entitity = {'entity':[], 'offset': 0}
        for i, id in enumerate(sample_predictions):
            # entities pipeline
            if id == self.dataset.ner_to_id['O']:
                if len(entitity['entity']) > 0:
                    entitity['entity'] = self.dataset.tokenizer.decode(entitity['entity'])
                    entity_copy = deepcopy(entitity)
                    if 'text' in sentence:
                        text = sentence['text']
                        final_offset = entitity['offset']
                        try:
                            final_offset = len(text[:final_offset-12]) + text[final_offset-12:].index(entitity['entity'])
                            entity_copy['offset'] = final_offset
                        except:
                            pass
                    result['entities'].append(entity_copy)
                    entitity = {'entity':[], 'offset': 0}

            # elif    id == self.dataset.ner_to_id['B'] and \
            #         len(entitity['entity']) > 0:

            #     entitity['entity'] = self.dataset.tokenizer.convert_ids_to_tokens(entitity['entity'])
            #     result['entities'].append(deepcopy(entitity))
            #     entitity = {'entity':[], 'offset': 0}
                
            #     entitity['offset'] = len( self.dataset.tokenizer.convert_ids_to_tokens( input_ids[1:i+1] ) )
            #     entitity['entity'].append(input_ids[i])

            else:
                if len(entitity['entity']) == 0:
                    entitity['offset'] = len( self.dataset.tokenizer.decode( input_ids[1:i] ) )
                entitity['entity'].append(input_ids[i])

        # ner output
        sample_predictions_formatted = []
        assert len(sample_output_mask) == len(sample_predictions)
        for is_valid, prediction in zip(sample_output_mask, sample_predictions):
            if is_valid:
                sample_predictions_formatted.append(prediction)

        result['ner'] = [ self.dataset.id_to_ner[e] for e in sample_predictions_formatted ]
