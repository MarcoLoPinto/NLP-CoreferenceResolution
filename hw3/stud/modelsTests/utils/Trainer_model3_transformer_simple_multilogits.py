...
class Trainer_model3_transformer_simple_multilogits(Trainer_model3):
    ...
    def compute_forward(self, model, sample, device, optimizer = None):
        ...
        # other:
        predicted_entities = sample['predicted_entities'].to(device)

        # infos useful for output:
        words_ids = sample['words_ids']

        # outputs:
        labels = sample['gold_entities'].to(device)
